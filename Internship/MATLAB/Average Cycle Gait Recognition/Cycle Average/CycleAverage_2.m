...
for i=1:length(locs)-1
    for j=1:length(locs)-1
        [distances(i,j), bla1, bla2, bla3] = dtw(cycles{i}, cycles{j});
    end
end

%% Calculate the average of rows
row_avgs = sum(distances)' ./(length(distances(1, :)));
[min_value, min_index] = min(row_avgs);
avg_gait_cycle = cycles{min_index};
